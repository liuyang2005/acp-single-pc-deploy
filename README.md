...
- `execute` 只运行一个 action chunk 的前 4 个稀疏点；连续模式在自由空间每执行
  2 点便采集新观测并立即重规划。仅当增量接触力达到 `5 N`，且预测末端相对当前
  TCP 明确上升至少 `30 mm` 时，才保留该 16 点计划并依次执行
  `0-1、2-3...14-15`，完成接触阶段。
...
`continuous-dry-run` 会真实连接机器人并自动归位，但不会发送任何策略位姿。自由空间中
每预览 2 个动作点便重新采集腕部图像、位姿与 wrench，并按严格递增的 request ID
调用推理；每个新计划都从 point 0 开始。只有真实增量接触力达到 `5 N` 且预测末端
向上至少 `30 mm` 时，才触发 16 点计划承诺。承诺后不再插入新推理，而是用单一连续
时间轴完整预览这 16 点并以 `dry_run_commitment_complete` 进入 HOLD；若没有触发承诺，
120 秒后以 `runtime_limit_reached` 结束。必须检查 `action_plan_commitment_check`、
`action_plan_time_scaling`、腕部 RGB、推理延迟、刚度、等效目标与工作空间预测；任何异常
都不能进入真实连续执行。
...
逐控制周期生效。接触计划会依据 reference/virtual 的最大线速度、线加速度、角速度和
角加速度自动放慢，只使用配置上限的 80%；若所需时间倍率超过 `6.0`，会直接 FAULT，
不会超速追赶。16 点执行完成后，控制器持续保持最后一点，TCP 在 `10 mm` 和 `0.10 rad`
内收敛即以 `committed_plan_complete` 进入 HOLD；8 秒仍未收敛则以
`commitment_settle_timeout` 进入 HOLD 并返回非零状态。它不会恢复 point 0--1 的向下
重规划。连续运动总上限仍为 120 秒；`Ctrl+C` 是正常操作员停止，物理紧急停止始终是
最高优先级。
