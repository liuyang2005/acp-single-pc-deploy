...
自由空间中每个新计划从 point 0 开始；只有真实增量接触力达到 `5 N` 且预测末端向上
至少 `30 mm` 时，才触发 16 点计划承诺。运行上限是 120 秒，正常结束原因为
`runtime_limit_reached`。必须检查 `action_plan_commitment_check`、腕部 RGB、推理延迟、
刚度、等效目标与工作空间预测；任何异常都不能进入真实连续执行。
